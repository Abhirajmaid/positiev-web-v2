...
export function Button({
  ...
  const neuBtn = formTheme === 'dark' ? 'form-neu-btn-dark border-0' : formTheme === 'light' ? 'form-neu-btn-light border-0' : '';
  const base =
    'group relative inline-flex items-center justify-center overflow-hidden font-medium transition-all duration-300 rounded-xl px-6 py-3 text-base md:text-lg ' +
    (neuBtn || (variants[variant] || variants.primary)) +
    (className ? ' ' + className : '');
